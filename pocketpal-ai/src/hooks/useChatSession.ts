...
import {OffAnswer} from '../off/router';
...
export const useChatSession = (
  ...
  // Accumulate unsaid text per message so we only speak complete sentences once
  const pendingSpeechRef = useRef<Record<string, string>>({});
      ...
      if (context && tokensToProcess.length > 0) {
        ...
        for (const [id, combinedToken] of Object.entries(messageUpdates)) {
          try {
            ...
            // Speak complete sentences as they arrive (Chat flow)
            if (uiStore.speakResponsesEnabled) {
              const enders = ['.', '!', '?', '…', '。', '！', '？'];
              const currentBuffer = pendingSpeechRef.current[id] ?? '';
              const newBuffer = currentBuffer + combinedToken;
              // Find last sentence end in the accumulated buffer
              let lastEnd = -1;
              for (let i = 0; i < newBuffer.length; i++) {
                if (enders.includes(newBuffer[i])) lastEnd = i;
              }
              if (lastEnd >= 0) {
                ...
                pendingSpeechRef.current[id] = newBuffer;
              }
            }
          } catch (error) {
  ...
  const handleSendPress = async (message: MessageType.PartialText) => {
    // OFF Mode shortcut: if enabled, bypass LLM and render OffMessage
    try {
      const offMode = false; // placeholder; actual toggle handled in ChatScreen wrapper
      if ((message as any).metadata?.offMode) {
        const started = Date.now();
        const off = await routeToOff(message.text);
        const elapsed = Date.now() - started;
        if (__DEV__) {
          console.log('[OFF] routeToOff ms:', elapsed, 'url:', getLastOffUrl());
        }

        const customMessage: MessageType.Custom = {
          author: assistant,
          createdAt: Date.now(),
          id: '',
          type: 'custom',
          ...
          // We'll reuse text field for serialization fallback; UI renders via renderCustomMessage
          text: '',
        } as any;

        await addMessage(customMessage);

        // After adding the shell, update to embed data for custom renderer if needed
        return;
      }
    } catch (e: any) {
      const msg = String(e?.message || e || 'OFF request failed');
      if (msg.includes('429') || msg.toLowerCase().includes('rate')) {
        await addSystemMessage("We're temporarily rate-limited, please try again in a few seconds.");
      } else {
        await addSystemMessage(`OFF error: ${msg}`);
      }
      return;
    }
    ...
    try {
      ...
      // Speak any remaining buffered text at end when enabled
      if (uiStore.speakResponsesEnabled && result?.text) {
        const msgId = currentMessageInfo.current?.id;
        if (msgId) {
