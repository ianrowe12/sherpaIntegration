...
export const VideoPalScreen = observer(() => {
  ...
  const handleImageCapture = useCallback(
    async (imageBase64: string) => {
      ...
      // Avoid starting a new completion while the context is busy
      // This mitigates intermittent "Context is busy" errors when frames arrive faster than inference completes
      if (modelStore.inferencing || modelStore.isStreaming) {
        return;
      }
        ...
        await modelStore.startImageCompletion({
          ...
          onToken: token => {
            ...
            if (!isStoppingCamera) {
              ...
              // Speak complete sentences from the delta as they arrive
              if (uiStore.speakResponsesEnabled) {
                const currentSpoken = spokenChars;
                const delta = built.slice(currentSpoken);
                if (delta) {
                  const enders = ['.', '!', '?', '…', '。', '！', '？'];
                  let lastEnd = -1;
                  for (let i = 0; i < delta.length; i++) {
                    if (enders.includes(delta[i])) {
                      lastEnd = i;
                    }
                  }
                  if (lastEnd >= 0) {
                    const toSpeak = delta.slice(0, lastEnd + 1).trim();
                    if (toSpeak) {
                      speakText(toSpeak).catch(() => {});
                      setSpokenChars(currentSpoken + lastEnd + 1);
                    }
                  }
                }
              }
          ...
          onComplete: () => {
            ...
            // Speak the accumulated response when enabled
            if (uiStore.speakResponsesEnabled) {
              const remaining = built.slice(spokenChars).trim();
              if (remaining) {
                speakText(remaining).catch(() => {});
                setSpokenChars(built.length);
              }
            }
          },
          onError: error => {
