...
describe('input', () => {
  ...
  it('sends a text message if defaultValue is provided', () => {
    ...
    fireEvent.press(button);
    expect(onSendPress).toHaveBeenCalledWith({text: defaultValue, type: 'text'});
    expect(textInput.props).toHaveProperty('value', defaultValue);
  });

  it('shows the nutrition placeholder when nutrition mode is enabled', () => {
    ...
    expect(
      getByPlaceholderText(
        l10n.en.components.chatInput.nutritionPlaceholder,
      ),
    ).toBeTruthy();
  });

  it('sends a text message when defaultValue is provided', () => {
    expect.assertions(2);
    const onSendPress = jest.fn();
    const defaultValue = 'defaultValue';
    const {getByPlaceholderText, getByLabelText} = render(
