...
class TTSManager: RCTEventEmitter, AudioPlayerDelegate {
    ...
    private func resolveModelPath(_ path: String) -> String {
        ...
        let resourceDirectory = directory.isEmpty ? nil : directory
    ...
    func initializeSTT(_ modelId: String) {
        ...
        var model = sherpaOnnxOfflineModelConfig(
            tokens: resolveModelPath(tokens),
            transducer: sherpaOnnxOfflineTransducerModelConfig(
                encoder: resolveModelPath(encoder),
                decoder: resolveModelPath(decoder),
                joiner: resolveModelPath(joiner)
            ),
            numThreads: 1,
            provider: "cpu",
            debug: 1
